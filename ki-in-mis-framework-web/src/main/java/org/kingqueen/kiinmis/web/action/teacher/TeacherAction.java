...
public class TeacherAction {
	...
	public ResponseDatagrid findCoach(RequestDatagrid datagrid){
		...
		return biz.findTeacher(datagrid,knumber);
	}
	...
	public ResponseEntity<byte[]> derivation(String path) throws Exception {
		...
		List<String> kiinids = indexBiz.calculateUserManagementKiin(user);
		String knumber=kiinids.get(0).toString();
		List<TeacherVo> payoutPeriodViewLists = biz.selAll(null,knumber);
	...
	public ResponseEntity<byte[]> derivationSelWhere(String path,
		...
		List<String> kiinids = indexBiz.calculateUserManagementKiin(user);
		String knumber=kiinids.get(0).toString();
		List<TeacherVo> payoutPeriodViewLists = biz.selAll(payoutPeriodViewList,knumber);
		return establishXSL(payoutPeriodViewLists, path);
	}
	...
	private ResponseEntity<byte[]> establishXSL(
		...
		Label labelB = new Label(1, 0, "姓名", normalFormat);
		Label labelC = new Label(2, 0, "性别", normalFormat);
		...
		for (int i = 0; i < payoutPeriodViewLists.size(); i++) {
			...
			if(payoutPeriodViewList.getCoachInductionTime()!=null){
			...
			if(payoutPeriodViewList.getCoachBirthDate()!=null){
			...
			lab12 = new Label(11, i + 1,
					payoutPeriodViewList.getCoachingState()==0?"正常":"注销");
			}else{
			 lab12 = new Label(11, i + 1,
						"");
			}
	...
	* @throws Exception
	...
	public String print(@RequestBody String jsonString) {
		...
		for (TeacherVo p : paymentViewLists) {
			...
			if(p.getCoachInductionTime()!=null){
				StudentAdmissionTime= sdf.format(date);  //2015-02-09  format()才是格式化
			}
			if(p.getCoachBirthDate()!=null){
			StudentBirthDate= sdf.format(date1);  //2015-02-09  format()才是格式化
			}
			String sex="";
